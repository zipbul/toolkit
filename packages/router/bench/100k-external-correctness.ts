...
type Probe = {
  ...
  expect:
    | { kind: 'no-match' }
    | { kind: 'match'; value: number; params?: Record<string, string> };
};
...
function runScenario(scenarioName: string, routes: Array<[string, string, number]>, probes: Probe[], skipFor: string[] = []): void {
  ...
  for (const a of adapters) {
    ...
    for (const probe of probes) {
      let res: any;
      try { res = a.match(r, probe.method, probe.path); } catch (e) {
        fail++;
      ...
      if (probe.expect.kind === 'no-match') {
        if (res === null) pass++;
        else { fail++; fails.push(`${probe.method} ${probe.path} → expected no-match, got ${JSON.stringify(res).slice(0, 60)}`); }
      } else {
        if (res === null) {
          fail++;
        ...
        else { fail++; fails.push(`${probe.method} ${probe.path} → value=${res.value}, params=${JSON.stringify(res.params)} (expected ${probe.expect.value}, ${JSON.stringify(probe.expect.params)})`); }
      }
    }
...
runScenario('param-1k', paramRoutes, [
  ...
  { method: 'GET', path: '/tenant-500/users/abc/posts/xyz', expect: { kind: 'match', value: 500, params: { user: 'abc', post: 'xyz' } } },
...
runScenario('wildcard-100', wildcardRoutes, [
  ...
  { method: 'GET', path: '/files/group-99/deep/nested/path/file.bin', expect: { kind: 'match', value: 99, params: { path: 'deep/nested/path/file.bin' } } },
]);

// ─── 4. Wrong method ───
runScenario('wrong-method', [
  ['GET', '/x', 1],
  ['POST', '/y', 2],
], [
