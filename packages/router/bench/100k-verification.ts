...
function mixedScenario(): Scenario {
  ...
  for (let i = 0; i < COUNT; i++) {
    const mod = i % 4;
    if (mod === 0) {
      routes.push(['GET', `/v${i % 20}/static/resource-${i}`, i]);
    } else if (mod === 1) {
      routes.push(['GET', `/v${i % 20}/users/:id/items/${i}`, i]);
    } else if (mod === 2) {
      routes.push(['POST', `/v${i % 20}/orgs/:org/repos/:repo/actions/${i}`, i]);
    } else {
      routes.push(['GET', `/v${i % 20}/files/${i}/*path`, i]);
    }
  }
...
function wildcardConflictFeasibility(): void {
  ...
  for (const size of sizes) {
    ...
    for (let i = 0; i < size; i++) {
      routes.push(['GET', `/wc/${i}/*path`, i]);
    }
    for (let i = 0; i < size; i++) {
      routes.push(['GET', `/static/${i}/leaf`, i]);
    }
