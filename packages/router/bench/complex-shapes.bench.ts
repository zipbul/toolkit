...
function buildZipbul(shape: Shape): Built | null {
  switch (shape) {
    ...
    case 'heavy-static': {
      ...
      let id = 0;
      for (let i = 0; i < 100; i++) {
        r.add('GET', `/api/v1/sys/cfg${i}`, id++);
      }
      for (let i = 0; i < 200; i++) {
        r.add('GET', `/api/v1/users${i}/:userId`, id++);
      }
      for (let i = 0; i < 100; i++) {
        r.add('GET', `/api/v1/orgs${i}/:org/repos/:repo`, id++);
      }
      for (let i = 0; i < 100; i++) {
        r.add('GET', `/api/v1/projects${i}/:proj/issues/:issue/comments/:comment`, id++);
      }
      r.build();
      return {
    ...
    case 'heavy1k-regex': {
      ...
      let id = 0;
      for (let i = 0; i < 200; i++) {
        r.add('GET', `/static/page${i}`, id++);
      }
      for (let i = 0; i < 200; i++) {
        r.add('GET', `/users${i}/:id`, id++);
      }
      for (let i = 0; i < 200; i++) {
        r.add('GET', `/orgs${i}/:org/repos/:repo`, id++);
      }
      for (let i = 0; i < 100; i++) {
        r.add('GET', `/search${i}/:q([a-z]+)`, id++);
      }
      for (let i = 0; i < 100; i++) {
        r.add('GET', `/files${i}/*path`, id++);
      }
      for (let i = 0; i < 200; i++) {
        r.add('GET', `/api${i}/v1/users/:id/posts/:post/comments/:c`, id++);
      }
      r.build();
      const benchUrl =
...
function buildMemoirist(shape: Shape): Built | null {
  switch (shape) {
    ...
    case 'heavy1k-regex': {
      ...
      for (let i = 0; i < 100; i++) {
        r.add('GET', `/search${i}/:q`, id++);
      }
      for (let i = 0; i < 100; i++) {
        r.add('GET', `/files${i}/*`, id++);
      }
      for (let i = 0; i < 200; i++) {
        r.add('GET', `/api${i}/v1/users/:id/posts/:post/comments/:c`, id++);
      }
...
function buildRou3(shape: Shape): Built | null {
  switch (shape) {
    ...
    case 'heavy-static': {
      ...
      let id = 0;
      for (let i = 0; i < 100; i++) {
        addRoute(r, 'GET', `/api/v1/sys/cfg${i}`, id++);
      }
      for (let i = 0; i < 200; i++) {
        addRoute(r, 'GET', `/api/v1/users${i}/:userId`, id++);
      }
      for (let i = 0; i < 100; i++) {
        addRoute(r, 'GET', `/api/v1/orgs${i}/:org/repos/:repo`, id++);
      }
      for (let i = 0; i < 100; i++) {
        addRoute(r, 'GET', `/api/v1/projects${i}/:proj/issues/:issue/comments/:comment`, id++);
      }
      return {
    ...
    case 'heavy1k-regex': {
      ...
      let id = 0;
      for (let i = 0; i < 200; i++) {
        addRoute(r, 'GET', `/static/page${i}`, id++);
      }
      for (let i = 0; i < 200; i++) {
        addRoute(r, 'GET', `/users${i}/:id`, id++);
      }
      for (let i = 0; i < 200; i++) {
        addRoute(r, 'GET', `/orgs${i}/:org/repos/:repo`, id++);
      }
      for (let i = 0; i < 100; i++) {
        addRoute(r, 'GET', `/search${i}/:q`, id++);
      }
      for (let i = 0; i < 100; i++) {
        addRoute(r, 'GET', `/files${i}/**:path`, id++);
      }
      for (let i = 0; i < 200; i++) {
        addRoute(r, 'GET', `/api${i}/v1/users/:id/posts/:post/comments/:c`, id++);
      }
      const benchUrl =
        shape === 'heavy1k-static'
          ? HEAVY1K_STATIC_URL
