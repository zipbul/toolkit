/**
 * Direct unit specs for the per-branch emit helpers extracted from
 * `emitNode`. Each helper returns a plain JS string fragment; we assert
 * the exact substrings so a regression in any one fragment surfaces
 * here instead of through a downstream walker mismatch.
 */
