...
describe('Parameter Naming Strictness (Bun Only)', () => {
  it('should allow snake_case and camelCase', () => {
    ...
    r.add('GET', '/:user_id', 1);
    r.add('GET', '/:postTitle', 2);
    r.add('GET', '/:v1_beta', 3);
    
    expect(() => r.build()).not.toThrow();
    expect(r.match('GET', '/42')?.params.user_id).toBe('42');
    expect(r.match('GET', '/hello')?.params.postTitle).toBe('hello');
  });

  it('should reject kebab-case', () => {
    const r = new Router<number>();
    r.add('GET', '/:user-id', 1);
    expect(() => r.build()).toThrow(/Only alphanumeric characters and underscores/);
  });
  ...
  it('should reject names starting with a digit', () => {
    const r = new Router<number>();
    r.add('GET', '/:123id', 1);
    expect(() => r.build()).toThrow(/must start with a letter/);
  });
