...
describe('RouterErrorKind reproducers (full coverage of 22 kinds)', () => {
  ...
  it('path-non-ascii', () => {
    expectKindOnBuild(r => r.add('GET', '/한국어', 'v'), 'path-non-ascii');
  });
  ...
  it('path-encoded-control', () => {
    expectKindOnBuild(r => r.add('GET', '/foo/%01/bar', 'v'), 'path-encoded-control');
  });
  ...
  it('regex-unsafe', () => {
    ...
    expectKindOnBuild(r => r.add('GET', '/users/:id((a+)+b)', 'v'), 'regex-unsafe');
  });
