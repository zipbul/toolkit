...
| Integration | `*.test.ts` | `test/` | 모듈 간 결합 |
...
```
Rule: TST-HERMETIC
Violation: I/O·시간·랜덤 등 비결정적 자원이 SUT 경계 안에서 mock 없이 사용됨
Enforcement: block
```
...
- **Integration**: 공개(exported) API만 사용.
...
```
Rule: TST-ACCESS
Violation: Integration 테스트가 unexported 멤버를 __testing__ export 없이 직접 접근
Enforcement: block
```
...
Violation: 디렉토리에 *.spec.ts가 1개 이상 존재하고,
          ...
          (*.d.ts, *.spec.ts, *.test.ts, index.ts, types.ts 제외)
Enforcement: block
```
